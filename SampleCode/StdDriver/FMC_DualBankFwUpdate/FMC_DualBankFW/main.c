...
void SYS_Init(void)
{
    ...
    /* Select Timer 0 clock source from HIRC */
...
void start_timer0()
{
    ...
    /* start timer */
    TIMER0->CTL = (47 << TIMER_CTL_PSC_Pos) | TIMER_ONESHOT_MODE | TIMER_CTL_CNTEN_Msk;
}
...
void RecevieAndSendBack(UART_T * tUART)
{
    ...
    /* setting before test */
    UART_WAIT_TXEMPTYF_RXIDLE(UART0);

    UART_Init(tUART, UART_CLK_SRC_HIRC, UART_CLK_DIV(1), 0);

    /* baud rate setting */
    tUART->BAUD = UART_BAUD_MODE2 | UART_BAUD_MODE2_DIVIDER(__HIRC, 115200);
    u32TestBR = 115200;
...
int32_t main(void)
{
    ...
    while(1)
    {
        ...
        if(i32ch == 'y')
        {
             ...
             *  Verify ...
             */
            FMC_Erase(BANK1_FW_CRC_BASE & FMC_PAGE_ADDR_MASK);
