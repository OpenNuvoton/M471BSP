...
{
    /* Unlock register due to CLK_PowerDown() */
    SYS_UnlockReg();
...
void CIR_Init(void)
{
    ...
    CIR_EnableInt(CIR0, CIR_INTCTL_PERRIEN_Msk|CIR_INTCTL_EPMIEN_Msk|CIR_INTCTL_CMPMIEN_Msk|CIR_INTCTL_PDWKIEN_Msk);

    CIR_SetInputType(CIR0, CIR_POSITIVE_EDGE, CIR_INVERSE);
    NVIC_EnableIRQ(CIR_IRQn);

    /* Error bypass function needs to be enabled if power down mode.                                           */
    /* Due to error flag will cause that CIR stop to convert data. It will be dangerous in power down mode     */
    /* And it will caused system can not be woken up by specified pattern                                      */
    /* CIR will clear error flag if meet header pattern for pattern type 0 and 1                               */
    /* CIR also clear error flag if pattern type 2 without header pattern in power down mode after end pattern */
...
void CIR_Parsing(void)
{
    ...
    do
    {
        while(isEmpty() != 1)    /* Queue is not empty */
        {
            u32IrCode = Pop(Queue);
            if(u32IrCode != 0xFFFFFFFF)     /* IR code is not repeat code */
            {
