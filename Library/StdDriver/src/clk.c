...
/**
  ...
  * | :----------------  | :----------------------------------- | :----------------------- |
  * |\ref TRACE_MODULE   | x                                    | \ref CLK_CLKDIV4_TRACE(x)|
  ...
  * |\ref CIR0_MODULE    |\ref CLK_CLKSEL2_CIR0SEL_HXT          | x                        |
  * |\ref CIR0_MODULE    |\ref CLK_CLKSEL2_CIR0SEL_LXT          | x                        |
  * |\ref CIR0_MODULE    |\ref CLK_CLKSEL2_CIR0SEL_TM0          | x                        |
  * |\ref CIR0_MODULE    |\ref CLK_CLKSEL2_CIR0SEL_LIRC         | x                        |
  * |\ref CIR0_MODULE    |\ref CLK_CLKSEL2_CIR0SEL_HIRC         | x                        |
...
uint32_t CLK_EnablePLL(uint32_t u32PllClkSrc, uint32_t u32PllFreq)
{
    ...
    /* Constraint: PLL output frequency must <= 240MHz */
    /*             PLL output frequency must > 50.14MHz to meet all constraints */
    u32PLL_UpperLimit = FREQ_240MHZ;
...
lexit:
    ...
    if(u32PllClkSrc == CLK_PLLCTL_PLLSRC_HXT)
        CLK->PLLCTL = CLK_PLLCTL_120MHz_HXT;
    else
        CLK->PLLCTL = CLK_PLLCTL_120MHz_HIRC_DIV4;
